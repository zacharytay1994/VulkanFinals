...
namespace RENDERER
{
	Vulkan::Vulkan (
		...
		:
		m_instance ( appName , config ) ,
		m_surface ( hwnd , m_instance.m_instance ) ,
		m_devices ( m_instance.m_instance , m_surface.m_surface , config ) ,
		m_swapchain ( m_devices.m_physical_device , m_devices.m_logical_device , m_surface.m_surface ) ,
		...
		m_descriptor ( m_devices )
	{
		...
		{
			std::cerr << "[ERROR!] RENDERER::Vulkan - Failed to initialize sync objects!" << std::endl;
		}
	}
	...
	void Vulkan::BindGraphicsPipeline ( std::string const& name )
	{
	...
	void Vulkan::AddModel ( std::string const& name , VK_OBJECT::Vertices const& vertices , VK_OBJECT::Indices const& indices )
	{
		...
		{
			std::cout << "[SUCCESS] RENDERER::Vulkan::AddModel - " << name << " model added successfully." << std::endl;
		}
		else
		{
			std::cout << "[ERROR!] RENDERER::Vulkan::AddModel - " << name << " model adding failed!" << std::endl;
		}
	}

	void Vulkan::AddModel ( std::string const& name , std::string const& fbx )
	{
		assert ( m_models.find ( name ) == m_models.end () );
		m_models.insert ( { name, { m_devices, m_graphics_queue, m_commands.m_command_pool, fbx.c_str () } } );
	...
	void Vulkan::AddImage ( std::string const& name , std::string const& png )
	{
	...
	void Vulkan::BindImage ( std::string const& name )
	{
	}

	void Vulkan::Draw ( std::string const& model , float x , float y , float z , float rx , float ry , float rz , float sx , float sy , float sz )
	{
	...
	void Vulkan::StartFrame ()
	{
		...
		{
			std::cerr << "[ERROR!] RENDERER::Vulkan::Draw - Failed to begin command buffer." << std::endl;
		}
	...
	void Vulkan::EndFrame ()
	{
		...
		{
			std::cerr << "[ERROR!] RENDERER::Vulkan::Draw - Failed to end command buffer." << std::endl;
		}
	...
	bool Vulkan::InitializeSyncObjects ()
	{
		...
		for ( size_t i = 0; i < VK_HELPER::MAX_FRAMES_IN_FLIGHT; ++i )
		{
			...
			{
				std::cerr << "[ERROR!] RENDERER::Vulkan::InitializeSyncObjects - Failed to create semaphore for a frame." << std::endl;
				return false;
			}
		}
		std::cout << "[SUCCESS] RENDERER::Vulkan::InitializeSyncObjects - Sync objects initialized successfully." << std::endl;
		return true;
	}
